...
class XLSX:
    def __init__(self, path: str, headList: list, sheetNum=0):
        ...
        xlsx类别\n
        :param path: xlsx文件路径
        :param headList: 要写入的表头
        :param sheetNum: 选择第几个表
        """
        self.path = path
    ...
    def writeLine(self, line: list):
        ...
        for i in range(len(line)):
            self.writeXlsx(row=row, col=i + 1, value=line[i])
        self.save()
...
if __name__ == '__main__':
    ...
    head = ["地区", "店铺链接", "店铺名", "地址", "电话", "证件号码", "企业名称", "商品链接", "商品名", "原价",
            "优惠价"]
    xlsx = XLSX(xlsxPath, head)
    a = 1
